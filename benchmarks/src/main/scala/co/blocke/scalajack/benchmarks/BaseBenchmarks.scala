...
class BaseBenchmarksState {
  ...
  val listOfPersons = flexScalaJack.read[List[Person]](jsonString, vc)
...
class BaseBenchmarks {
  ...
  @Benchmark
  def readNativeScalaJack(state: BaseBenchmarksState): List[Person] = {
    state.nativeScalaJack.read[List[Person]](state.jsonString, state.vc)
  }

  @Benchmark
  def readFlexScalaJack(state: BaseBenchmarksState): List[Person] = {
    state.flexScalaJack.read[List[Person]](state.jsonString, state.vc)
  }

  @Benchmark
  def writeNativeScalaJack(state: BaseBenchmarksState): String = {
    state.nativeScalaJack.render[List[Person]](state.listOfPersons, state.vc)
  }

  @Benchmark
  def writeFlexScalaJack(state: BaseBenchmarksState): String = {
    state.flexScalaJack.render[List[Person]](state.listOfPersons, state.vc)
  }
