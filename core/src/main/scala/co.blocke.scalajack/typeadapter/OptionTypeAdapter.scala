...
object OptionTypeAdapter extends TypeAdapterFactory.=:=.withOneTypeParam[Option] {
  ...
  override def create[E](next: TypeAdapterFactory)(implicit context: Context, tt: TypeTag[Option[E]], ttElement: TypeTag[E]): TypeAdapter[Option[E]] = {
    ...
    OptionTypeAdapter(new OptionIRTransceiver(valueTypeAdapter.irTransceiver), valueTypeAdapter)
  }
...
case class OptionTypeAdapter[T](override val irTransceiver: IRTransceiver[Option[T]], valueTypeAdapter: TypeAdapter[T]) extends TypeAdapter[Option[T]] {
