...
case class VisitorContext(
    ...
    customAdapters:  List[flexjson.TypeAdapterFactory] = List.empty[flexjson.TypeAdapterFactory],
  ...
  def withAdapter(ta: flexjson.TypeAdapterFactory) = this.copy(customAdapters = customAdapters :+ ta)
}