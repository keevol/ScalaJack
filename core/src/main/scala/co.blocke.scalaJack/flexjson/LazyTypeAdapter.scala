...
case class LazyTypeAdapter[T](context: Context, tpe: Type, typeArgs: List[Type]) extends TypeAdapter[T] {
  ...
  @inline def resolve(): TypeAdapter[T] = {
    ...
    if (typeAdapter == null) {
      typeAdapter = context.typeAdapter(tpe, typeArgs).asInstanceOf[TypeAdapter[T]]
