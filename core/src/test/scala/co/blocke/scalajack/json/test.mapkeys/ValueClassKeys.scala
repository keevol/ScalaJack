  ...
  describe("------------------------------\n:  ValueClass Map Key Tests  :\n------------------------------") {
    ...
    describe("--- Negative Primitive Tests ---") {
      ...
      it("Bad Enumeration Key") {
        val js = """{"m":{"\"Bogus\"":"Meat"}}"""
        val msg = """ReadException(1 error):
                      |  [$.m."Bogus"] Enumeration co.blocke.scalajack.json.test.mapkeys.Food$ does not contain a value named "Bogus" (reported by: co.blocke.scalajack.typeadapter.EnumerationIRTransceiver)""".stripMargin
